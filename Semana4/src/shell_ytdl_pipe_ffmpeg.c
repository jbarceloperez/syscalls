...
int main(int argc, char *argv[])
{
    ...
    case 0: /* Hijo derecho de la tubería  */
        ...
        // redirigir la salida de error del hijo derecho al fichero dado, 3 PASOS: ABRIR, DUP, CERRAR
        int fderr;
        fderr = open("ffmpeg.log", O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);    //ABRIR FICHERO, OBTENIENDO DESCRIPTOR
        if (fderr == -1)
        {
            perror("open(fderr)");
            exit(EXIT_FAILURE);
